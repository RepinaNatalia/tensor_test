...
class Trigon:
    """Функция проверяет, подходят ли длины для треугольника"""
    def __init__(self, *atr):
        if len(atr) != 3:
            n = len(atr)
            raise IndexError(f'Передано {n} аргументов, а ожидается 3')
        elif len([i for i in atr if type(i) == int]) != 3:
            raise TypeError('Стороны должны быть числами')
        elif atr[0] + atr[1] <= atr[2] and atr[1] + atr[2] <= atr[0] and atr[0] + atr[2] <= atr[1]:
            raise Exception('Не треугольник')
        elif len([i for i in atr if i > 0]) != 3:
