...
def everything_for_your_cat(cats_data):
    """функция принимает список кортежей, из каждого кортежа вычленяет первые 2 элемента - имя кота и возраст, затем к добавляет их к 2 последним элементам, затем создает словарь, где ключ - имя фамилия, значение кот и возраст"""
    dict = {}  #  создаем пустой словарь
    our_str = '' # создаем пустую строку
    for i in cats_data:  # вычленяем каждый элемент списка
        cats = i[0] + ', ' + str(i[1])  # из каждого кортежа составляем строку "кот, возраст"
        dict.setdefault(i[2:], []).append(cats)  # добавляем владельца и кота с возрастом. если есть владелец,то только кота
