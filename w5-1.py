...
def letter_stat(our_str):
    """принимает число i и строку, возвращает ключ - значение индекса i в строке и значение - число повторений ключа в строке"""
    i = 0
    d = len(our_str)
    letters_dict = {} #   создаем пустой словарь
    while i != d:
        letters_dict[str(our_str[i])] = our_str.count(our_str[i]) #добавляем в словарь ключ - символ строки и значение - число вхождений
        i += 1
    return letters_dict
