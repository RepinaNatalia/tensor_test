...
class PersonInfo:
    def __init__(self, name:str, age:int, *dep:list):
        self.name = name
        self.age = age
        self.dep = dep

    def short_name (self):
        b = self.name.find(" ")   # ищем индекс пробела
        c = self.name[(b + 1):] + " " + self.name[0] + "."   #   срез - от пробела до конца и первая буква имени
        return(c)
    def path_deps(self):
        b = ' --> '.join([i for i in (self.dep)])   #   соединяем отделы через  -->
        return(b)

    def new_salary(self):
        b = ""
        for i in range(len(self.dep)):    #   соединяем отделы в 1 строку
            b += str(self.dep[i])
            i += 1

        dict = {}
        for i in range(len(b)):   #   составляем словать: символ - число вхождеений
            dict[b[i]] = b.count(b[i])
        print(dict)
        x = list(dict.values())   #   составляем список из значением словаря
        l = sorted(x)   #   сортируем список
        K = int(l[-1]) + int(l[-2]) + int(l[-3])   #   складываем 3 последние значения - получаем наш коэффициент
        sal = 1337 * K * self.age
        return (sal)
